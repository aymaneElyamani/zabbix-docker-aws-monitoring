...
- Zabbix déployé en conteneurs Docker (server + web + DB Postgres)
- Monitoring des 2 clients (statut ZBX vert, graph CPU/RAM, démo alerte)

> Remarque : ce dépôt contient déjà un `docker-compose.yml` prêt à l’emploi (Zabbix 7.0 + Postgres).
...
- Une IP publique “stable” (ton réseau actuel) pour limiter les accès (règle SG « My IP »).
...
1. AWS Console → **VPC** → Create VPC (option **VPC and more** conseillée)
...
Inbound (entrants) :
- HTTP `80` depuis **ton IP** (accès UI Zabbix)
- HTTPS `443` depuis **ton IP** (optionnel si tu configures TLS)
- Zabbix server `10051` depuis le SG des clients (ou depuis le CIDR du VPC)
...
> Ce dépôt mappe l’UI Zabbix sur le port `80` de l’EC2 (mapping `80:8080` côté conteneur).
...
- Numéroter chaque image + légende : “Figure X : …”
...
- AWS : montrer 3 instances “Running” + tags
- Zabbix :
  1. Login UI
  2. **Data collection → Hosts** (agents connectés)
...
- Région : **us-east-1**
- Après redémarrage EC2, relancer : `docker compose up -d`
